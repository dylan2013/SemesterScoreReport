...
namespace SemesterScoreReport
{
    public partial class SemesterScoreReportFormNew : SelectSemesterForm
    {
        ...
        public MemoryStream Template
        {
            get
            {
                if (_useDefaultTemplate)
                    return new MemoryStream(忠信學年成績單.Properties.Resources._97前入學_普綜_學年成績單);
        ...
        {
            XmlElement config = SmartSchool.Customization.Data.SystemInformation.Preference["_97前入學_普綜_學年成績單"];
            if (config != null)
            {
                //使用者設定的假別
                _userType.Clear();
                ...
                else
                {
                    ...
                    SmartSchool.Customization.Data.SystemInformation.Preference["_97前入學_普綜_學年成績單"] = config;
                }
            ...
            {
                #region 產生空白設定檔
                config = new XmlDocument().CreateElement("_97前入學_普綜_學年成績單");
                SmartSchool.Customization.Data.SystemInformation.Preference["_97前入學_普綜_學年成績單"] = config;
                #endregion
            }
        }
        ...
        {
            SelectTypeForm form = new SelectTypeForm("_97前入學_普綜_學年成績單");
