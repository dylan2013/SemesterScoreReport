...
namespace SemesterScoreReport
{
    class SemesterScoreReport
    {
        public static void RegistryFeature()
        {
            ...
            string reportName = "97前入學_職_學年成績單";
            string path = "忠信高中";
        ...
        private void _BGWSemesterScoreReport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (StudentRecord eachStudent in allStudent)
            {
                ...
                mergeKeyValue.Add("下學期取得學分數", thisSemesterTotalCredit);
                mergeKeyValue.Add("下學期累計取得學分數", beforeSemesterTotalCredit);
                ...
                mergeKeyValue.Add("上學期成績名次", rating.GetPlace(schoolyear, 1));

                mergeKeyValue.Add("下學期學業成績", secondScore);
                mergeKeyValue.Add("下學期成績名次", rating.GetPlace(schoolyear, 2));

                mergeKeyValue.Add("學年學業成績", thisSchoolYearScore);
                mergeKeyValue.Add("學年成績名次", ratingSchoolYear.GetPlace(schoolyear));                
        ...
        private void MailMerge_MergeField(object sender, Aspose.Words.Reporting.MergeFieldEventArgs e)
        {
            ...
            if (e.FieldName == "科目")
            {
                ...
                foreach (SemesterSubjectScoreInfo info in sortList)
                {
                    ...
                    SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 1].Paragraphs[0].Runs.Add(new Run(e.Document, subjectScore[info]["必修"]));
                    SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 1].Paragraphs[0].Runs[0].Font.Size = 10;
                    SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 2].Paragraphs[0].Runs.Add(new Run(e.Document, subjectScore[info]["學分"]));
                    SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 2].Paragraphs[0].Runs[0].Font.Size = 10;

                    if (Convert.ToDecimal(subjectScore[info]["分數"]) < resitStandardSchoolYear[info.GradeYear])
                        SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 3].Paragraphs[0].Runs.Add(new Run(e.Document, subjectScore[info]["分數"] + " " + resitSign));
                    else
                        SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 3].Paragraphs[0].Runs.Add(new Run(e.Document, subjectScore[info]["分數"]));

                    SSTable.Rows[SSTableRowIndex].Cells[SSTableColIndex + 3].Paragraphs[0].Runs[0].Font.Size = 10;
